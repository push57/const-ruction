...
const Working = () => {
  return (
    <div className='w-full bg-white' id='working'>
      ...
      <motion.h1 variants={slideUpVariants} className='text-black-[42px] font-bold text-3xl' >HOW IT'S WORKING</motion.h1>
     ...
     items-start gap-[20px] mt-[30px]'>
    {
      planning.map ((item,index) =>(
        <div key={index} className='flex flex-col justify-center items-center gap-5 border-2 border-yellow-500 rounded-mg p-6' >
          ...
          <p className='text-[18px] text-center'>{item.about}</p>
          </div>
